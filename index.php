...
function content($content) {
    ...
    $count = [];
    return \preg_replace_callback('/<(caption|div|figcaption|h[1-6]|p|summary)(\s(?:"[^"]*"|\'[^\']*\'|[^>])*)?>([\s\S]*?)<\/\1>/i', static function ($m) use (&$count) {
...
\Hook::set('page.content', __NAMESPACE__ . "\\content", 2.1); // Make sure to run this hook before `x\t_o_c\content` hook
